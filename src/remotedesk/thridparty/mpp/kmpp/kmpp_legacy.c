 ...
 *
 * kmpp_legacy.c - Legacy /dev/vcodec path operations
 */

#define  MODULE_TAG "kmpp_legacy"
...
static MPP_RET stop(Kmpp *ctx)
{
    ...
    ret = mpp_vcodec_ioctl(ctx->mClientFd, VCODEC_CHAN_STOP, 0, 0, 0);
    if (ret)
        mpp_err("chan %d VCODEC_CHAN_STOP failed\n", ctx->mChanId);
