...
typedef Vepu5xxRoiH264BsCfg Vepu510RoiH264BsCfg;

typedef Vepu51xH264Fbk Vepu510H264Fbk;
...
static MPP_RET hal_h264e_vepu510_start(void *hal, HalEncTask *task)
{
    ...
    do {
        ...
        vepu510_dump_sw_regs(ctx->dbg_ctx, regs);
        ...
        if (hal_dbg_flag_en(ctx->dbg_ctx, HAL_DBG_GET_REG)) {
            RK_S32 ret_dbg = 0;
            vepu510_get_dbg_regs(ctx->dev, regs, ret_dbg);
            if (ret_dbg)
...
static MPP_RET hal_h264e_vepu510_wait(void *hal, HalEncTask *task)
{
    ...
    vepu510_dump_hw_regs(ctx->dbg_ctx, regs, regs->reg_st);
