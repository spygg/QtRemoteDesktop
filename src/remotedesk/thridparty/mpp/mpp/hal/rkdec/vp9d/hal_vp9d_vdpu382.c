...
static MPP_RET hal_vp9d_alloc_res(HalVp9dCtx *hal)
{
    ...
    if (p_hal->fast_mode) {
        for (i = 0; i < VDPU_FAST_REG_SET_CNT; i++) {
            hw_ctx->g_buf[i].hw_regs = mpp_calloc_size(void, sizeof(Vdpu382RegSet));
        ...
        hw_ctx->hw_regs = mpp_calloc_size(void, sizeof(Vdpu382RegSet));
...
static void hal_vp9d_rcb_info_update(void *hal,  Vdpu382RegSet *hw_regs, void *data)
{
    HalVp9dCtx *p_hal = (HalVp9dCtx*)hal;
    Vdpu382Vp9dCtx *hw_ctx = (Vdpu382Vp9dCtx*)p_hal->hw_ctx;
    DXVA_PicParams_VP9 *pic_param = (DXVA_PicParams_VP9*)data;
...
static MPP_RET hal_vp9d_vdpu382_gen_regs(void *hal, HalTaskInfo *task)
{
    ...
    /* set info for multi core */
    {
        ...
        vp9_hw_regs->comm_gen.reg028.sw_poc_arb_flag = 1;
    ...
    vp9_hw_regs->comm_gen.reg011.err_head_fill_e  = 1;
    vp9_hw_regs->comm_gen.reg011.err_colmv_fill_e = 1;

    vp9_hw_regs->comm_gen.reg026.inter_auto_gating_e = 1;
    vp9_hw_regs->comm_gen.reg026.filterd_auto_gating_e = 1;
    vp9_hw_regs->comm_gen.reg026.strmd_auto_gating_e = 1;
    vp9_hw_regs->comm_gen.reg026.mcp_auto_gating_e = 1;
    vp9_hw_regs->comm_gen.reg026.busifd_auto_gating_e = 1;
    vp9_hw_regs->comm_gen.reg026.dec_ctrl_auto_gating_e = 1;
    vp9_hw_regs->comm_gen.reg026.intra_auto_gating_e = 1;
    vp9_hw_regs->comm_gen.reg026.mc_auto_gating_e = 1;
    vp9_hw_regs->comm_gen.reg026.transd_auto_gating_e = 1;
    vp9_hw_regs->comm_gen.reg026.sram_auto_gating_e = 1;
    vp9_hw_regs->comm_gen.reg026.cru_auto_gating_e = 1;
    ...
    hal_vp9d_rcb_info_update(hal, vp9_hw_regs, pic_param);
    {
        ...
        vdpu382_setup_rcb(&vp9_hw_regs->comm_addr, cfg->dev, rcb_buf, hw_ctx->rcb_info);
    }

    {
        ...
        if (mpp_frame_get_thumbnail_en(mframe)) {
            vp9_hw_regs->comm_addr.reg198_scale_down_luma_base =
                vp9_hw_regs->comm_addr.reg130_decout_base;
            vp9_hw_regs->comm_addr.reg199_scale_down_chorme_base =
                vp9_hw_regs->comm_addr.reg130_decout_base;
            vdpu382_setup_down_scale(mframe, cfg->dev, &vp9_hw_regs->comm_gen);
        } else {
            vp9_hw_regs->comm_addr.reg198_scale_down_luma_base = 0;
            vp9_hw_regs->comm_addr.reg199_scale_down_chorme_base = 0;
            vp9_hw_regs->comm_gen.reg012.scale_down_en = 0;
        }
    }
    vdpu382_setup_statistic(&vp9_hw_regs->comm_gen, &vp9_hw_regs->statistic);
...
static MPP_RET hal_vp9d_vdpu382_start(void *hal, HalTaskInfo *task)
{
    ...
    HalVp9dCtx *p_hal = (HalVp9dCtx*)hal;
    Vdpu382Vp9dCtx *hw_ctx = (Vdpu382Vp9dCtx*)p_hal->hw_ctx;
    Vdpu382RegSet *hw_regs = (Vdpu382RegSet *)hw_ctx->hw_regs;
    MppDev dev = p_hal->cfg->dev;

    if (p_hal->fast_mode) {
        RK_S32 index =  task->dec.reg_index;
        hw_regs = (Vdpu382RegSet *)hw_ctx->g_buf[index].hw_regs;
    }
...
static MPP_RET hal_vp9d_vdpu382_wait(void *hal, HalTaskInfo *task)
{
    ...
    if (p_hal->fast_mode)
        hw_regs = (Vdpu382RegSet *)hw_ctx->g_buf[task->dec.reg_index].hw_regs;
    ...
    if (hal_vp9d_debug & HAL_VP9D_DBG_REG) {
        ...
        for (i = 0; i < sizeof(Vdpu382RegSet) / 4; i++)
